...
export function HyvorTalkPanel({ result }: HyvorTalkPanelProps) {
  if (!result.found && !result.error)
    return (
      <div id="hyvor-talk" className="p-2 bg-yellow-100 border border-yellow-500">
  ...
  if (result.error) {
    return (
      <div
        id="hyvor-talk"
        className="rounded-md border border-amber-400/50 bg-amber-50 dark:bg-amber-950/40 px-4 py-3 text-sm text-amber-900 dark:text-amber-200"
      >
  ...
  return (
    <div
      id="hyvor-talk"
      className="flex flex-col gap-3 rounded-md border border-black/10 dark:border-white/10 p-4"
    >
